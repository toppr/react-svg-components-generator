...
function generator(options) {
    ...
    const iconComponentTemplate = fs.readFileSync(path.join(templatesDir, 'iconComponent.nunjucks'), 'utf8');
    const iconBaseComponentTemplate = fs.readFileSync(path.join(templatesDir, 'iconBaseComponent.nunjucks'), 'utf8');
    ...
    glob(svgSrcDir + '/**/*.svg', async (err, svgPaths) => {
        ...
        for (let svgPath of svgPaths) {
            ...
            if (!_.includes(_.toLower(componentName), 'colored')) {
            ...
            fs.outputFileSync(
                ...
                nunjucks.renderString(iconComponentTemplate, {
                    ...
                    iconBaseComponentPath: `./${path.relative(path.parse(componentPath).dir, iconBaseComponentPath)}`,
